...
namespace DALLib.File
{
	...
	public class TEXFile : FileBase, IDisposable
	{
		...
		public override void Load(ExtendedBinaryReader reader, bool keepOpen = false)
		{
			...
			if (useLZ77)
				SheetData = SheetData.DecompressLZ77(UseBigEndian);
		...
		public void DumpFrames(string dir)
		{
			int i = 0;
			float fw = (float)Bmp.Width;
			float fh = (float)Bmp.Height;
			foreach (var f in Frames)
			{
				int x0 = (int)(f.LeftScale * fw);
				int x1 = (int)(f.RightScale * fw);
				int y0 = (int)(f.TopScale * fh);
				int y1 = (int)(f.BottomScale * fh);
				Bitmap bmp = new Bitmap((int)f.FrameWidth, (int)f.FrameHeight);
				for (int y = 0; y < bmp.Height; y++)
				{
					for (int x = 0; x < bmp.Width; x++)
					{
						bmp.SetPixel(x, y, Bmp.GetPixel(x0 + x, y0 + y));
					}
				}

				bmp.Save(dir + i.ToString() + ".png");
				i++;
			}
		}
	}
